...
@section('title', __('garage::dashboard.garages.routes.index'))
@section('content')
  ...
  <div class="page-content">
    <div class="page-bar">
      <ul class="page-breadcrumb">
        ...
        <li>
          <a href="#">{{__('garage::dashboard.garages.routes.index')}}</a>
        </li>
      </ul>
    </div>
        ...
        <div class="portlet light bordered">
          ...
          <div class="table-toolbar">
            <div class="row">
              <div class="col-md-6">
                <div class="btn-group">
                  <a href="{{ url(route('dashboard.garages.create')) }}" class="btn sbold green">
          ...
          <div class="portlet-title">
            <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">
                {{__('garage::dashboard.garages.routes.index')}}
              </span>
            </div>
          </div>
            ...
            <table class="table table-striped table-bordered table-hover" id="dataTable">
              <thead>
                <tr>
                  ...
                  <th>#</th>
                  <th>{{__('garage::dashboard.garages.datatable.title')}}</th>
                  <th>{{__('garage::dashboard.garages.datatable.image')}}</th>
                  <th>{{__('garage::dashboard.garages.datatable.status')}}</th>
                  <th>{{__('garage::dashboard.garages.datatable.created_at')}}</th>
                  <th>{{__('garage::dashboard.garages.datatable.options')}}</th>
                </tr>
              </thead>
            </table>
          </div>
          <div class="row">
            <div class="form-group">
              <button type="submit" id="deleteChecked" class="btn red btn-sm" onclick="deleteAllChecked('{{ url(route('dashboard.garages.deletes')) }}')">
  ...
  function tableGenerate(data = '') {
            ...
            var dataTable =
                $('#dataTable').DataTable({
                    ...
                    ajax: {
                        url: "{{ url(route('dashboard.garages.datatable')) }}",
                        type: "GET",
                        data: {
                            req: data,
                        },
                  ...
                  `;
                        ...
                        },
                        {
                            ...
                            width: '13%',
                            title: '{{__('garage::dashboard.garages.datatable.options')}}',
                            className: 'dt-center',
                            orderable: false,
                            render: function (data, type, full, meta) {

                                // Edit
                                var editUrl = '{{ route("dashboard.garages.edit", ":id") }}';
                                editUrl = editUrl.replace(':id', data);

                                // Delete
                                var deleteUrl = '{{ route("dashboard.garages.destroy", ":id") }}';
