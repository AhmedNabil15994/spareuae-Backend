...
<script>
  ...
  function deleteRow(url, msg = "") {
    ...
    if (!msg) msg = '{{ __('apps::dashboard.messages.delete') }}'
    bootbox.confirm({
      message: msg,
      buttons: {
        confirm: {
  ...
  function deleteAllChecked(url) {
    ...
    var selectedItems = $("input:checkbox:checked");
  ...
  $(document).ready(function() {
    ...
    $('#reportrange').daterangepicker({
      ...
      ranges: {
        ...
        '{{ __('apps::dashboard.buttons.datapicker.last_month') }}': [moment().subtract(1, 'month').startOf('month'), moment()
...
<script>
  ...
  pusher.subscribe('ilcovo-channel').bind('activity-log', function(data) {
