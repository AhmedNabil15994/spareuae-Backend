...
class CompanyRepository
{
    ...
    {
        $count  = $this->user->companyType()->count();
        return ["count" => $count];
    }

    public function countUsers($order = 'id', $sort = 'desc')
    {
        $users = $this->user->companyType()->count();
    ...
    */
    ...
    {
        $users = $this->user->companyType()->select("id,name")->orderBy($order, $sort)->get();
        return $users;
    }
    ...
    */
    ...
    {
        $users = $this->user->companyType()->active()->orderBy($order, $sort)->select("id", "name", "type")->get();
        return $users;
    }
    ...
    */
    ...
    {
        $users = $this->user->active()->companyType();
        if ($request->search) {
    ...
    */
    ...
    {
        $query = $this->user->withDeleted()->where('id', '!=', auth()->id())->companyType()->where(function ($query) use ($request) {
