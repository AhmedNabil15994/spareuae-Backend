...
@section('title', __('user::dashboard.offices.index.title'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('user::dashboard.offices.index.title')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.users.create')) }}" class="btn sbold green">
                    ...
                    <div class="portlet-title">
                        <div class="caption font-dark">
                            ...
                            <span class="caption-subject bold uppercase">
                                {{__('user::dashboard.offices.index.title')}}
                            </span>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('user::dashboard.offices.datatable.image')}}</th>
                                    <th>{{__('user::dashboard.offices.datatable.name')}}</th>
                                    <th>{{__('user::dashboard.offices.datatable.email')}}</th>
                                    <th>{{__('user::dashboard.offices.datatable.mobile')}}</th>
                                    <th>{{__('user::dashboard.offices.datatable.status')}}</th>
                                    <th>{{__('user::dashboard.offices.datatable.type')}}</th>


                                    <th>{{__('user::dashboard.offices.datatable.created_at')}}</th>
                                    <th>{{__('user::dashboard.offices.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.offices.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columns: [
            ...
            {data: 'email' 	          , className: 'dt-center'},
                  ...
                  {data: 'type' 	        , className: 'dt-center',  render: function(data, type, row){
                        if(data == "user")
                          return '<span class="badge badge-success"> {{__('user::dashboard.offices.datatable.user')}} </span>';
                        return '<span class="badge badge-info"> {{__('user::dashboard.offices.datatable.office')}} </span>';
                    }},
 ...
 responsivePriority:1,
              width: '13%',
              title: '{{__('user::dashboard.offices.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {
                       ...
                       // Edit
                        ...
                        //show
                        var showUrl = '{{ route("dashboard.users.show", ":id") }}';
