...
@section('title', __('user::dashboard.technicals.routes.index'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('user::dashboard.technicals.routes.index')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.technicals.create')) }}" class="btn sbold green">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('user::dashboard.technicals.datatable.image')}}</th>
                                    <th>{{__('user::dashboard.technicals.datatable.name')}}</th>
                                    <th>{{__('user::dashboard.technicals.datatable.email')}}</th>
                                    <th>{{__('user::dashboard.technicals.datatable.mobile')}}</th>
                                    <th>{{__('user::dashboard.technicals.datatable.status')}}</th>
                                    <th>{{__('user::dashboard.technicals.datatable.type')}}</th>


                                    <th>{{__('user::dashboard.technicals.datatable.created_at')}}</th>
                                    <th>{{__('user::dashboard.technicals.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                onclick="deleteAllChecked('{{ url(route('dashboard.technicals.deletes')) }}')">
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.technicals.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columns: [
            ...
            {data: 'email' 	          , className: 'dt-center'},
                  ...
                  {data: 'type' 	        , className: 'dt-center',  render: function(data, type, row){
                        return '<span class="badge badge-info"> {{__('user::dashboard.technicals.datatable.technical')}} </span>';
                    }},
 ...
 responsivePriority:1,
              width: '13%',
              title: '{{__('user::dashboard.technicals.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                       // Edit
      					var editUrl = '{{ route("dashboard.technicals.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);
                        //show
                        var showUrl = '{{ route("dashboard.technicals.show", ":id") }}';
      					showUrl = showUrl.replace(':id', data);
      					// Delete
      					var deleteUrl = '{{ route("dashboard.technicals.destroy", ":id") }}';
