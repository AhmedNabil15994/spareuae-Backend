...
@section('title', __('user::dashboard.workers.index.title'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('user::dashboard.workers.index.title')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.workers.create')) }}" class="btn sbold green">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('user::dashboard.workers.datatable.image')}}</th>
                                    <th>{{__('user::dashboard.workers.datatable.name')}}</th>
                                    <th>{{__('user::dashboard.workers.datatable.email')}}</th>
                                    <th>{{__('user::dashboard.workers.datatable.mobile')}}</th>
                                    <th>{{__('user::dashboard.workers.datatable.vendor_id')}}</th>
                                    <th>{{__('user::dashboard.workers.datatable.status')}}</th>
                                    <th>{{__('user::dashboard.workers.datatable.branch_id')}}</th>

                                    <th>{{__('user::dashboard.workers.datatable.created_at')}}</th>
                                    <th>{{__('user::dashboard.workers.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                onclick="deleteAllChecked('{{ url(route('dashboard.workers.deletes')) }}')">
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.workers.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columns: [
            ...
            {data: 'email' 	          , className: 'dt-center'},
                  ...
                  {data: 'vendor_id' 	        , className: 'dt-center'},
                  {data: 'is_active' 	        , className: 'dt-center'},
                  {data: 'branch_id' 	        , className: 'dt-center'},
 ...
 responsivePriority:1,
              width: '13%',
              title: '{{__('user::dashboard.workers.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.workers.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

                          var showUrl = '{{ route("dashboard.workers.show", ":id") }}';
      					showUrl = showUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.workers.destroy", ":id") }}';
