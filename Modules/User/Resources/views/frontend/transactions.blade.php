      ...
      <div class="d-table-cell">
        <div class="container">
          <div class="page-banner-content">
            <h2>{{trans('user::frontend.profile.transactions')}}</h2>
            <ul class="pages-list">
              <li><a href="{{URL::to('/')}}">{{trans('apps::frontend.pages.home.title')}}</a></li>
              <li><span>{{trans('user::frontend.profile.transactions')}}</span></li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="row">
        ...
        <div class="col-lg-9 col-md-12">
          <div class="dashboard-title">
            <h3>{{trans('user::frontend.profile.transactions')}}</h3>
          </div>
          ...
          <div class="row">
            <table class="table table-bordered">
              <thead>
                <tr>
                  <th>ID</th>
                  <th>{{trans('user::frontend.profile.transactions_form.description')}}</th>
                  <th>{{trans('user::frontend.profile.transactions_form.total')}}</th>
                  <th>{{trans('user::frontend.profile.transactions_form.status')}}</th>
                  <th>{{trans('user::frontend.profile.transactions_form.created_at')}}</th>
                </tr>
              </thead>
              <tbody>
                @foreach($transactions as $transaction)
                  <tr>
                    ...
                    <td>{{ $transaction->order_type != null && str_contains($transaction->order_type,'Subscription') ? trans('user::frontend.profile.transactions_form.package') :  trans('user::frontend.profile.transactions_form.order') }}</td>
