...
@section('title',__('offer::dashboard.offers.routes.index')))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('offer::dashboard.offers.routes.index')}}</a>
                </li>
            </ul>
        </div>
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                @can("add_offers")
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.offers.create')) }}" class="btn sbold green">
                                            ...
                                            <div class="form-body">
                                                <div class="row">
                                                    ...
                                                    {{-- <div class="col-md-3">
                                                        <div class="form-group">
                                                            <label class="control-label">
                                                                {{__('order::dashboard.orders.datatable.offer')}}
                                                            </label>
                                                            <select name="offer_id" id="single" class="form-control">
                                                                <option value="">
                                                                    {{__('apps::dashboard.datatable.form.select')}}
                                                                </option>
                                                                @foreach ($offers as $offer)
                                                                <option value="{{ $offer['id'] }}">
                                                                    {{ $offer->translate(locale())->title }}
                                                                </option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('offer::dashboard.offers.datatable.image')}}</th>
                                    <th>{{__('offer::dashboard.offers.datatable.title')}}</th>
                                    <th>{{__('offer::dashboard.offers.datatable.description')}}</th>
                                    <th>{{__('offer::dashboard.offers.datatable.start_at')}}</th>
                                    <th>{{__('offer::dashboard.offers.datatable.end_at')}}</th>
                                    <th>{{__('offer::dashboard.offers.datatable.percent')}}</th>
                                    <th>{{__('offer::dashboard.offers.datatable.created_at')}}</th>
                                    <th>{{__('offer::dashboard.offers.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                onclick="deleteAllChecked('{{ url(route('dashboard.offers.deletes')) }}')">
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.offers.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
            ...
            { data: "image" ,orderable: false , width: "1%",
              render: function(data, type, row){
                return '<img src="'+data+'" width="50px"/>'
                  ...
                  {data: 'percent' 	      , className: 'dt-center'},
 ...
 responsivePriority:1,
              width: '13%',
              title: '{{__('offer::dashboard.offers.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.offers.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.offers.destroy", ":id") }}';
