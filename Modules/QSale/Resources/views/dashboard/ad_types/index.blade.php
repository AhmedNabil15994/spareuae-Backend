...
@section('title', __('qsale::dashboard.ad_types.routes.index'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('qsale::dashboard.ad_types.routes.index')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.ad_types.create')) }}" class="btn sbold green">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('qsale::dashboard.ad_types.datatable.icon')}}</th>
                                    <th>{{__('qsale::dashboard.ad_types.datatable.name')}}</th>
                                    <th>{{__('qsale::dashboard.ad_types.datatable.description')}}</th>

                                    <th>{{__('qsale::dashboard.ad_types.datatable.status')}}</th>
                                    <th>{{__('qsale::dashboard.ad_types.datatable.created_at')}}</th>
                                    <th>{{__('qsale::dashboard.ad_types.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                onclick="deleteAllChecked('{{ url(route('dashboard.ad_types.deletes')) }}')">
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ajax : {
              url   : "{{ url(route('dashboard.ad_types.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
 ...
 responsivePriority:1,
              width: '13%',
              title: '{{__('qsale::dashboard.ad_types.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.ad_types.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.ad_types.destroy", ":id") }}';
