...
@section('title', __('question::dashboard.questions.routes.index'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('question::dashboard.questions.routes.index')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.questions.create')) }}" class="btn sbold green">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('question::dashboard.questions.datatable.question')}}</th>
                                    <th>{{__('question::dashboard.questions.datatable.answer')}}</th>
                                    <th>{{__('question::dashboard.questions.datatable.status')}}</th>
                                    <th>{{__('question::dashboard.questions.datatable.created_at')}}</th>
                                    <th>{{__('question::dashboard.questions.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                onclick="deleteAllChecked('{{ url(route('dashboard.questions.deletes')) }}')">
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.questions.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columns: [
            {data: 'id' 		 	        , className: 'dt-center'},
      			{data: 'id' 		 	        , className: 'dt-center'},
      			{data: 'question' 			      , className: 'dt-center'},
                  {data: 'answer' 			      , className: 'dt-center'},
 ...
 responsivePriority:1,
              width: '13%',
              title: '{{__('question::dashboard.questions.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.questions.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.questions.destroy", ":id") }}';
